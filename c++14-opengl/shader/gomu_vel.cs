...
void main()
{
  ...
  // velocity verlet法による力と速度の更新(後半)
  vec3 force = vec3(f1 + f2 + m * g, 0.f);
  vec3 delta_vel = dt * force / (2.f * m);
  
  write_points[i].position = read_points[i].position;
  write_points[i].velocity = step(0.5, read_points[i].position.w) * (read_points[i].velocity + delta_vel);
  write_points[i].force = step(0.5, read_points[i].position.w) * force;
}
