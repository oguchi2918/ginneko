...
static SceneSolar2* scene = nullptr;
...
bool init(long n)
{
  ...
  // TODO:
  scene = new SceneSolar2(static_cast<unsigned>(n));
  if (!scene || !scene->init()) {
    return false;
  }
  //  vw = new nekolib::renderer::VideoWriter("./solar2.avi", SCREEN_WIDTH, SCREEN_HEIGHT);
...
int main(int argc, char* argv[])
{
  ...
  if (n <= 0 || n > 1000) {
    fprintf(stderr, "usage: solar2 N\n Argument N is point num. N must be >= 1 and <= 1000.\n");
    return -1;
  }
  if (!init(n)) {
    return -1;
