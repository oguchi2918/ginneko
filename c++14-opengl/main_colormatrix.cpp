...
static SceneColorMatrix* scene = nullptr;
...
bool init(void)
{
  ...
  // TODO:
  scene = new SceneColorMatrix();
  if (!scene) {
    return false;
  }
  int width, height;
  if (!scene->init(&width, &height)) {
    return false;
  }
  SDL_SetWindowSize(window, width, height);
  nekolib::renderer::ScreenManager::init(width, height);
